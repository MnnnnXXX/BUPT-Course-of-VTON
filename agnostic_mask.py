...
def get_img_agnostic(parse, pose_data):
    ...
    agnostic = Image.new('RGB', parse.size)
    ...
    agnostic_draw.rectangle((pointx - r * 7, pointy - r * 8, pointx + r * 7, pointy + r * 7), 'white', 'white')
    agnostic.paste(Image.new('RGB', parse.size), None, Image.fromarray(np.uint8(parse_head * 255), 'L'))
    agnostic.paste(Image.new('RGB', parse.size), None, Image.fromarray(np.uint8(parse_lower * 255), 'L'))
...
if __name__ == "__main__":
    data_path = "Output"
    output_path = "Output\\agnostic-mask"

    os.makedirs(output_path, exist_ok=True)

    for im_name in tqdm(os.listdir(osp.join(data_path, 'resized_image'))):
        ...
        # load parsing image
        im = Image.open(osp.join(data_path, 'resized_image', im_name))
        label_name = im_name.replace('.jpg', '.png')
        im_label = Image.open(osp.join(data_path, 'image-parse-v3', label_name))

        agnostic = get_img_agnostic(im_label, pose_data)
